...
?>
<div class="mb-0 <?php echo esc_attr( fw_ext_builder_get_item_width( 'form-builder', $item['width'] . '/frontend_class' ) ); ?>">
	<div class="form-group<?php echo( ( !empty( $attr['placeholder'] ) ) || ( ! empty( $options['icon'] ) ) ) ? esc_attr(' has-placeholder') : ''; ?>">
		...
		<?php endif; ?>
		<textarea
			class="form-control" rows="<?php echo esc_attr($options['rows_num']); ?>" <?php echo fw_attr_to_html( $attr ); ?>><?php echo fw_htmlspecialchars( $value ); ?></textarea>
		<?php if ( $options['info'] ): ?>
