...
(function ($) {
	...
	function processCompileButton(windowParent, wpCustomize) {
		...
		$parentBody.find('#sub-accordion-section-color_scheme_section').append('<li><a id="wp-scss-theme-recompile" class="button-primary">' + oktan_customizer_text.button_text + '</a></li>');
	...
	function compile(windowParent, wpCustomize) {
		...
		$.ajax({
			...
			var $cssLinkChild = $('#oktan-child-main-css');
			...
			var errorMessage = (typeof (msg.responseJSON) !== 'undefined') ? msg.responseJSON.data : oktan_customizer_text.error_text;
			preloaderHide();
